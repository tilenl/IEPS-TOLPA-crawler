...
### Why larger top-k and reranking matter (GitHub README structure)

The crawled repositories are **vast and very different** from one another, but their READMEs share a recurring layout: long **References**, **Citation**, and **Related work** sections next to the parts users care about (setup, training, evaluation). With **embedding retrieval alone**, asking for only **top-3** hits can therefore return **mostly bibliography-style segments**: those blocks are often long, lexically dense, and repeat query-related terms (method names, “segmentation”, dataset titles, paper keywords) many times, so the bi-encoder ranks them highly even when they are **not** the most *useful* answer to the question.

We therefore **increase top-k** (for example to **10** for inspection, and a **larger candidate pool** such as `--candidate-k 20` when reranking) so the same query still surfaces those reference-heavy hits, but also pulls in **segments from the explanatory body** of the README that sit slightly lower in pure vector order. **Cross-encoder reranking** then re-scores *(query, segment_text)* pairs and typically **pushes reference-style material toward the bottom of the list**, because it is less relevant than chunks that **describe** how the repository works or how to run the code—while segments that directly match the user’s intent move up. For GitHub-style READMEs, **using more top hits together with reranking is essential**: first-stage similarity alone is systematically biased toward citation-heavy text, and reranking is the practical way to recover actionable README content for *usage-oriented* questions.

We **did not** strip reference or citation blocks out of extraction, segmentation, or embeddings. Removing them would make a whole class of queries **impossible to ground**, for example **“which arXiv paper does this repository cite?”**—there would be nothing left to retrieve. We also assume **sources and referencing are crucial context** when this pipeline is used as **RAG** in front of an LLM: the model should still see bibliographic segments when they are relevant, and use them to **point the user to external reading** (arXiv links, paper titles, project pages) when the answer is not fully contained in the repository itself. A deliberately “cleaner” dataset with references cut out could justify **smaller top-k** and **less reranking** for narrow task-only retrieval, but would sacrifice those behaviours. We therefore kept the **full README content** in segments and vectors, and accepted **more top hits plus reranking** as the operational compromise instead of preprocessing references away.
