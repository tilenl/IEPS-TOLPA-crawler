...
### Local PostgreSQL Setup with Docker (Recommended)

This setup follows the same Docker workflow used in `vaje-1 Web crawling - basic tools.ipynb`, but initializes this project's real schema from `pa1/db/crawldb.sql`.
...
2. Docker daemon is running.
   ...
   - On macOS (if Docker daemon is not active): `colima start` (as shown in the notebook), then run `docker ps` again.
3. You execute commands from the repository root (`IEPS-TOLPA-crawler`), so relative paths resolve correctly.
...
Run a PostgreSQL container similarly to the notebook approach:
...
- `-d pgvector/pgvector:pg17`: runs in detached mode using the same image family as in the tutorial notebook.

> ⚠️ Initialization scripts in `/docker-entrypoint-initdb.d` run only when the data directory is empty (first container initialization).
...
- **Port `5432` already occupied**
  ...
  - Fix: publish another host port, e.g. `-p 5433:5432`, and update crawler DB connection config accordingly.
