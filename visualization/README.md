...
Edit `REAL_VIZ_DB_*` in `.env` so they match the running crawler (same values as `crawler.db.*` in [`pa1/crawler/src/main/resources/application.properties`](../pa1/crawler/src/main/resources/application.properties), or your env overrides).

The server and seed script load `.env` from the `visualization/` directory automatically via `dotenv` (you can still `source .env` in the shell if you prefer).
...
In the UI, choose profile **mock** (saved in the browser after you change it).
...
2) Copy `.env.example` to `.env` and set `REAL_VIZ_DB_*` to match the crawler database (host, port, user, password, database name).
...
You can still pass env vars inline instead of `.env` if needed:
...
The UI defaults to profile **real** (and remembers the last choice). The API defaults to `profile=real` when the query parameter is omitted.

Confirm the database is reachable: `GET http://localhost:3001/health` or `GET http://localhost:3001/health?profile=real` should return `{"ok":true,"profile":"real"}`.
...
- **Mock mode**:
  ...
  - select profile **mock** in the UI
- **Real mode**:
  - skip `npm run seed`
  - set `REAL_VIZ_DB_*` in `.env` (or exports) to match the crawler
  - select profile **real** in the UI (default)
...
TLS (optional, for managed Postgres / cloud):

- `VIZ_DB_SSL` or `MOCK_VIZ_DB_SSL` / `REAL_VIZ_DB_SSL`: `true` / `1` / `require` to enable TLS
- `PGSSLMODE`: `require`, `verify-ca`, or `verify-full` also enables TLS when no explicit `VIZ_DB_SSL` is set
- `VIZ_DB_SSL_REJECT_UNAUTHORIZED`: set to `false` only for self-signed certificates (development)
...
- `GET /graph` - graph payload (`nodes`, `edges`)
  - `profile` (`mock|real`, default `real`)
