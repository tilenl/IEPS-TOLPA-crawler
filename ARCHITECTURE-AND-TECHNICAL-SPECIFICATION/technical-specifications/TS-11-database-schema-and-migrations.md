...
| `V003__page_data_metadata_schema_v3.sql` | Historical: added `TITLE` / `META_DESCRIPTION` to `data_type` and unique index on `page_data(page_id, data_type_code)`; superseded by **V006** for fresh baselines (`crawldb.sql` no longer seeds those types). |
...
| `V006__remove_page_data_title_meta_schema_v6.sql` | Remove `TITLE` / `META_DESCRIPTION` `page_data` rows and `data_type` codes; bump `schema_version` to **6**. (V003 metadata types are historical only.) |
