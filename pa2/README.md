...
1. **Optional: build canonical cleaned-content mapping** (skip in our current pipeline, because duplicate pages were already removed during crawling):
  ```bash
   cd pa2/implementation-extraction
...
The script includes **3 expected-good** domain queries:
...
and **3 intentionally weak/hard** queries:
