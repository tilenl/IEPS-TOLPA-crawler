...
| Jsoup | DOM parser for deterministic extraction of `body a[href]`, `body [onclick]`, and `img[src]` candidates via CSS selectors. | `technical-specifications/TS-04-parser-and-extraction-specification.md`, `technical-specifications/TS-03-fetcher-specification.md` | Mandatory for parser stage; regex only for parsing redirect payload inside `onclick`. |
